...
const HomePage = () => {
  ...
  useEffect(() => {
    const fetchWaitlistCount = async () => {
      try {
        const response = await fetch("https://gravyn-backend.eu-north-1.elasticbeanstalk.com/api/waitlist/count");
...
const DesktopLayout = ({ bannerVisible, setBannerVisible, badgeVisible, setBadgeVisible, count, targetCount }) => {
  ...
  const handleJoinWaitlist = async () => {
    ...
    try {
      const response = await fetch("https://gravyn-backend.eu-north-1.elasticbeanstalk.com/api/waitlist", {
        method: "POST",
        headers: {
